  ...
  return (
    <header
      className={cn('sticky top-0 z-50 w-full border-b border-transparent transition-all duration-300', {
        'bg-sea-900/95 supports-[backdrop-filter]:bg-sea-900/50 border-emerald-500/20 backdrop-blur-lg shadow-lg': scrolled,
        'bg-sea-900 shadow-md': !scrolled,
      })}
    >
      <nav className="mx-auto flex h-16 w-full max-w-7xl items-center justify-between px-6">
        <div className="hover:bg-sea-800 rounded-[100px] py-1 px-4 flex items-center gap-2 cursor-pointer transition-colors">
          <span className="font-bold text-2xl tracking-wider text-emerald-400" style={{ fontFamily: "'Poppins', sans-serif" }}>SEALINE</span>
        </div>
        <div className="hidden items-center gap-4 md:flex">
          {links.map((link) => (
            <a key={link.label} className={cn(buttonVariants({ variant: 'ghost' }), "text-gray-100 hover:text-emerald-400 hover:bg-sea-800")} href={link.href}>
              {link.label}
            </a>
          ))}
          <a href="/login" className="px-5 py-2 rounded-full border border-emerald-500/30 text-emerald-400 hover:bg-emerald-500 hover:text-white transition-all font-medium text-sm">
            Sign In
          </a>
        </div>
        <Button
          ...
          className="md:hidden border-sea-700 bg-sea-800 text-white hover:bg-sea-700"
          aria-expanded={open}
      ...
      <MobileMenu open={open} className="flex flex-col justify-between gap-4">
        <div className="grid gap-y-2 mt-4">
          {links.map((link) => (
            <a
              ...
              }), "text-white hover:text-emerald-400 hover:bg-sea-800")}
              href={link.href}
            >
              {link.label}
            </a>
        ...
        <div className="flex flex-col items-center gap-3 mb-8 pt-4 border-t border-emerald-500/20">
          <a href="/login" className="w-full px-5 py-3 rounded-full bg-emerald-500 text-white hover:bg-emerald-400 transition-all font-medium text-center shadow-[0_4px_20px_rgba(16,185,129,0.3)]">
            Sign In
          </a>
        </div>
      </MobileMenu>
...
function MobileMenu({ open, children, className, ...props }) {
  ...
  return createPortal(
    <div
      id="mobile-menu"
      className={cn(
        'bg-sea-900/95 supports-[backdrop-filter]:bg-sea-900/80 backdrop-blur-xl',
        'fixed top-16 right-0 bottom-0 left-0 z-40 flex flex-col overflow-y-auto border-t border-emerald-500/20 md:hidden animate-in slide-in-from-top-2 duration-300'
      )}
    >
      <div
